...
export default function ProfilePage() {
    ...
    const handleProfileUpdate = async (e: React.FormEvent) => {
        e.preventDefault(); setUpdateLoading(true); setUpdateMessage("");
        try {
            const { data } = await api.put("/users/profile", { name, email, phone });
            login(data); setEditing(false); setUpdateMessage("Profile updated successfully!");
            setTimeout(() => setUpdateMessage(""), 3000);
        } catch (error: any) { setUpdateMessage(error.response?.data?.message || "Failed to update profile"); }
    ...
    const handlePasswordChange = async (e: React.FormEvent) => {
        ...
        } catch (error: any) { setPasswordError(error.response?.data?.message || "Failed to change password"); }
                ...
                <div className="md:grid md:grid-cols-4 md:gap-8">
                    ...
                    <div className="mb-8 md:mb-0">
                        <div className="card-premium p-6 text-center">
                            <div className="w-20 h-20 rounded-full mx-auto flex items-center justify-center text-3xl font-serif mb-4 bg-gradient-to-br from-gold-light/40 to-pink-blush/40 text-gold-dark border-2 border-gold/20">
                                {user.name.charAt(0)}
                            </div>
