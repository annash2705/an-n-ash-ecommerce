                ...
                <div className="card-premium py-8 px-6 sm:px-10">
                    ...
                    <form className="space-y-5" onSubmit={submitHandler}>
                        <div>
                            ...
                            <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="input-elegant" placeholder="Your name" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="input-elegant" placeholder="••••••••" />
                        </div>
                        <div className="pt-2">
                            <Button fullWidth type="submit" disabled={loading} size="lg">{loading ? "Creating..." : "Create Account"}</Button>
                        </div>
                    </form>
                    <div className="mt-8 text-center">
                        <div className="gold-divider mb-5"><div className="gold-divider-gem" /></div>
                        <p className="text-sm text-foreground/60">Already have an account?{' '}<Link href={`/login?redirect=${redirect}`} className="font-medium text-gold hover:text-gold-dark transition-colors">Sign in</Link></p>
                    </div>
                </div>
            </div>
        </div>
